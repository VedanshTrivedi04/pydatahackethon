...
## Phase 9 — NEXT: Sandbox Executor
**Will create**:
- `engine/core/sandbox/client.py` — API client to trigger an isolated Docker container or Firecracker microVM for running untrusted code generated by AI.
- Update `ModuleResult` to include sandbox execution logs (stdout/stderr).
- Wire it so modules (like `test_generator`) can execute generated code to verify it compiles.
